...
# Extract text chunks (split by headings if possible, or by paragraph blocks)
...
for p in paragraphs:
    if len(p) < 60 and (p.startswith("一、") or p.startswith("二、") or p.startswith("三、") or p.startswith("四、") or p.startswith("五、") or p.startswith("六、") or p.startswith("七、") or "：" in p[:15] or p.startswith("#")):
        ...
        if not current_slide["title"]:
            if not slides_data and len(p) < 100:
                current_slide["title"] = p
            else:
                current_slide["title"] = "詳細解析"
...
# If we have less than 15 slides, we need to split larger slides
final_slides = []
for slide in slides_data:
    title = slide["title"]
    content = slide["content"]
    
    # Split content if it has too many paragraphs
    chunk_size = 2
...
if len(final_slides) < 15:
    # Just force split by paragraph if still too few
    final_slides = []
    for slide in slides_data:
        title = slide["title"]
...
"""
    ...
    # Add an image every 3 slides just to make it "圖文並茂" (We will use placeholder or existing assets)
    if i % 3 == 1:
...
html_template += """
    ...
    <div class="controls">
        <button onclick="prevSlide()">&#9664; PREV (Left)</button>
        <button onclick="nextSlide()">NEXT (Right) &#9654;</button>
    </div>
