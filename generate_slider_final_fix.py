...
slides_data = []
current_slide = {"title": "技術細節與產業分析", "content": []}
char_count = 0
...
for sentence in sentences:
    ...
    if char_count > 400: # about 400 chars per slide
        slides_data.append(current_slide)
        current_slide = {"title": "技術細節與產業分析 (續)", "content": []}
        char_count = 0
    ...
    html_template += f"""
        <div class="slide">
            <div class="glass-panel">
                <h2>{slide['title']}</h2>
"""
    p_html = "".join([f"<p>{s}</p>" for s in slide['content']])
...
print(f"Generated successfully. Slides count: {len(slides_data) + 1}")
